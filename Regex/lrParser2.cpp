...
action LRParser::impl::step(grammar2::TerminalID tag) {
    ...
    if (tag.x >= action_table[s].size()) {
        std::stringstream ss;
        ss << "parse: invalid tag '" << grammar_.toString(tag) << "' in state\n";
        print(ss, grammar_, items[s]);
        ss << "action table:\n";
        for (auto i = 0; i < action_table[s].size(); ++i) {
            ss << grammar_.toString(grammar2::TerminalID(i)) << " -> ";
            print(ss, grammar_, action_table[s][i]);
            ss << '\n';
        }
...
const grammar2::Grammar& LRParser::grammar() const { return pimpl->grammar_; }

void LRParser::reset() { pimpl->reset(); }

action LRParser::step(grammar2::TerminalID tag) { return pimpl->step(tag); }
} // namespace lr_parser2
