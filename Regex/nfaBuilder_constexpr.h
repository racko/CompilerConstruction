...
class node_t {
  public:
    ...
    constexpr void addTransition(int64_t label, State destination) { ns[label].push_back(destination); }
...
class CountingGraph {
  public:
    ...
    constexpr void addEdge(State, State, int64_t) { ++edgeCount; }

    constexpr void addEpsilonEdge(State, State) { ++edgeCount; }
...
class TransitionCountingGraph {
  public:
    ...
    constexpr void addEdge(State source, State, int64_t) { ++ns_[source]; }

    constexpr void addEpsilonEdge(State source, State) { ++ns_[source]; }
...
class ResultingStateCountingGraph {
  public:
    ...
    constexpr void addEdge(State source, State, int64_t label) { ++ns_[source][label]; }

    constexpr void addEpsilonEdge(State source, State) { ++ns_[source][0]; }
...
class Graph {
  public:
    ...
    constexpr void addEpsilonEdge(State source, State destination) { ns_[source].addTransition(0, destination); }

    constexpr int64_t getStateCount() const { return ns_.size(); }

    constexpr void setKind(State state, TokenId kind) { ns_[state].setKind(kind); }
...
template <typename Symbol, typename State, int64_t MaxSymbols, typename Graph> // TODO: add default argument for Graph
                                                                               // as soon as I can compute the counts
                                                                               struct nfaBuilder {
