...
template<size_t MaxNodes, size_t MaxSymbols, typename Symbol = char, typename State = uint16_t, typename TokenId = uint8_t>
struct Lexer {
    const char* c{};
    using DFA_t = DFA<Symbol,State,TokenId,MaxNodes,MaxSymbols>;

    //constexpr Lexer(DFA_t dfa, Token<TokenId> eof, TokenId whitespace, const char* text) : dfa_(std::move(dfa)), eof_(eof), whitespace_(whitespace), c(text) {}
    //constexpr Lexer(DFA_t dfa, Token<TokenId> eof, TokenId whitespace) : dfa_(std::move(dfa)), eof_(eof), whitespace_(whitespace) {}
    constexpr Lexer(const DFA_t& dfa, Token<TokenId> eof, TokenId whitespace) : dfa_(dfa), eof_(eof), whitespace_(whitespace) {}
...
constexpr Token<TokenId> Lexer<MaxNodes,MaxSymbols,Symbol,State,TokenId>::getToken() {
    ...
    /// we need to check current for eof because eof is not in symbolToId ... or rather symbolToId[eof] == symbolToId.size()
