...
inline bool operator==(const TerminalID lhs, const TerminalID rhs) {
    return lhs.x == rhs.x;
}
...
inline bool operator==(const GrammarElement lhs, const GrammarElement rhs) {
    return lhs.x == rhs.x;
}
...
inline bool operator==(const NonterminalID lhs, const NonterminalID rhs) {
    return lhs.x == rhs.x;
}
...
public:
    size_t operator()(const grammar2::TerminalID a) const {
...
public:
    size_t operator()(const grammar2::NonterminalID A) const {
        return std::hash<decltype(A.x)>()(A.x);
    }
};
...
public:
    size_t operator()(const grammar2::GrammarElement X) const {
...
public:
    ...
    using Productions = std::vector<std::vector<String>>;
private:
...
public:
    ...
    const std::unordered_map<GrammarElement, std::unordered_set<TerminalID>>& getFirsts() const {
        return firsts_;
    }

    const std::unordered_set<TerminalID>& getFirsts(const GrammarElement X) const {
        return firsts_.at(X);
    }

    size_t getNumberOfGrammarElements() const {
        return getNumberOfTerminals() + getNumberOfNonterminals();
    }

    GrammarElement toGrammarElement(TerminalID t) const {
        return GrammarElement(t.x);
    }

    GrammarElement toGrammarElement(NonterminalID A) const {
        return GrammarElement(A.x + getNumberOfTerminals() + 1U);
    }
    ...
    NonterminalID toNonterminalID(GrammarElement X) const {
        ...
        return NonterminalID(X.x - getNumberOfTerminals() - 1U);
    }
    ...
    const char* toString(TerminalID t) const {
        return terminalStrings_[t.x];
    }
    const char* toString(NonterminalID A) const {
        return nonTerminalStrings_[A.x];
    }
...
void first(const Grammar& grammar, const std::vector<GrammarElement>& alpha, const std::unordered_map<GrammarElement, std::unordered_set<TerminalID>>& fs, std::unordered_set<TerminalID>& out);
