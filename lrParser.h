...
struct action {
  ...
  NonterminalID<G> getHead() const {
    ...
    auto shifted = x >> 15;
    auto mask = (1 << 15) - 1;
    return NonterminalID<G>(shifted & mask);
  }
  uint32_t getProduction() const { assert(getType() == type::REDUCE); return x & ((1 << 15) - 1); }
};

template<class G>
struct item {
  using String = std::list<GrammarElement<G>>;
  NonterminalID<G> A;
  String l;
  String r;
  TerminalID<G> a;
  ...
  item(NonterminalID<G> _A, String const& _l, String const& _r, TerminalID<G> _a, uint32_t _production) : A(_A), l(_l), r(_r), a(_a), production(_production) {}
};
...
struct LRParser {
  ...
  std::vector<std::unordered_map<TerminalID<G>, action<G>>> action_table;

  LRParser();
  virtual ~LRParser() {}
  template<class InputIterator>
  T parse(InputIterator w) const final;
  virtual T reduce(NonterminalID<G> A, uint32_t production, std::vector<T>&& alpha) const = 0;
  virtual T shift(typename G::Token_const_reference t) const = 0;
};
...
//      print(std::cout, G, i);
      const auto& B = NonterminalID<G>(i.r.front());
...
//        std::cout << std::endl;
        ...
        rest.push_back(GrammarElement<G>(i.a));
//        std::cout << "rest = ";
//        print(std::cout, G, rest);
//        std::cout << std::endl;
        auto firstRest = first(rest);
...
//          std::cout << "b = " << T(b) << std::endl;
          auto res = I.emplace(B, typename item<G>::String{}, typename item<G>::String{gamma.front() != GrammarElement<G>(G::eps) ? gamma.begin() : gamma.end(), gamma.end()}, b, k);
...
template <class G>
SetOfItems<G> GOTO(const SetOfItems<G>& I, GrammarElement<G> X) {
...
LRParser<G, T>::LRParser() {
  SetOfItems<G> init{item<G>(G::start, typename item<G>::String(), typename item<G>::String{G::getProductions(G::start)[0].begin(), G::getProductions(G::start)[0].end()}, G::eof, 0)};
  ...
  std::map<state,std::map<NonterminalID<G>,state>> _goto_table;
  std::map<state,std::map<TerminalID<G>,action<G>>> _action_table;
...
//    print(std::cout, G, I);
    for (auto x = 0u; x < G::getNumberOfGrammarElements(); ++x) {
      GrammarElement<G> X(x);
      if (G::kindOf(X) == kind::TERMINAL || G::kindOf(X) == kind::EOI) {
        for (const auto& item : I) {
          if (item.r.empty() && item.a == TerminalID<G>(X)) {
            action<G> a(type::FAIL);
            if (item.A == G::start)
              a = action<G>(type::ACCEPT);
            else
              a = action<G>(type::REDUCE, item.A, item.production);
            auto currentAction = _action_table[i].find(TerminalID<G>(X));
            auto gotConflict = currentAction != _action_table[i].end();
            if (!gotConflict) {
//              std::cout << "adding action " << X << " -> " << a << std::endl;
              _action_table[i].insert({TerminalID<G>(X), a});
            }else {
              std::cout << "conflict: ACTION(" << i << ", " << X << "): " << _action_table[i].find(TerminalID<G>(X))->second << " or " << a << std::endl;
...
//          std::cout << "adding goto " << X << " -> " << j << std::endl;
          _goto_table[i][NonterminalID<G>(X)] = j;
          ...
          auto gotConflict = currentAction != _action_table[i].end();
          if (gotConflict) {
            std::cout << "conflict: ACTION(" << i << ", " << X << "): " << _action_table[i].find(X)->second << " or " << a << std::endl;
            std::cout << "item set:" << std::endl;
            std::cout << I;
...
//  std::cout << C;
  ...
  goto_table.resize(C.size(), std::unordered_map<NonterminalID<G>, state>(G::getNumberOfNonterminals()));
  ...
  action_table.resize(C.size(), std::unordered_map<TerminalID<G>, action<G>>(G::getNumberOfTerminals()));
...
template<class G, class T>
template<class InputIterator>
T LRParser<G, T>::parse(InputIterator a) const {
  ...
  while(!stack.empty()) {
    ...
    if (_action.getType() == type::SHIFT) {
      ...
      attributes.push_back(shift(*a));
      std::cout << "Shift " << GrammarElement<G>(tag) << std::endl;
      ++a;
      ...
      size_t length;
      if (production.at(0) == GrammarElement<G>(G::eps))
        length = 0;
      else
        length = production.size();
      assert(stack.size() > length);
      auto synthesized_attribute = reduce(head, p, std::vector<T>(attributes.end() - length, attributes.end()));
      ...
      auto it = goto_table[stack.back()].find(head);
      if (it == goto_table[stack.back()].end()) {
        std::stringstream ss;
      ...
      stack.push_back(t);
      std::cout << "Reduce " << production << " to " << GrammarElement<G>(_action.getHead()) << std::endl;
