...
static constexpr const auto MaxResultStates = [] {
    ...
    return accumulate(transitionCounts.begin(), transitionCounts.end(), 0LL, [&] (auto a, const auto& b) { auto c = maxResultingStates(b); return std::max<int64_t>(a, c); });
}();

// TODO: MaxTransitions is not "Max Labels (symbols) per State".
// It's the total number of transitions per state (just as it's name says). But we would actually like to have the potentially lower number of max labels, because of the way the data structures we use structure the transitions.
...
auto min_dfa = [] {
    ...
    //    std::copy(min_dfa_.T.begin() + i * min_dfa_.symbolCount, min_dfa_.T.begin() + (i+1)*min_dfa_.symbolCount, std::ostream_iterator<int>(std::cout, ", "));
    //    std::cout << std::endl;
    //}
    //std::cout << "finals: ";
