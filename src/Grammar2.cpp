...
void first(TerminalID eps, const std::vector<GrammarElement>& alpha, const std::unordered_map<GrammarElement, std::unordered_set<TerminalID>>& fs, std::unordered_set<TerminalID>& out) {
...
std::unordered_map<GrammarElement, std::unordered_set<TerminalID>> allFirsts(const Grammar& grammar) {
    ...
    bool change;
    do change = updateFirsts(grammar, fs);
    while(change);
...
std::unordered_set<TerminalID> first(const Grammar& grammar, const std::vector<GrammarElement>& alpha) {
    ...
    first(grammar, alpha, grammar.getFirsts(), out);
    return out;
}
}
