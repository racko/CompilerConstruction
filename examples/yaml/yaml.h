...
struct TerminalID<Functional::Grammar> {
     ...
     constexpr TerminalID(Functional::T _x) : x(uint32_t(_x)) {}
...
struct NonterminalID<Functional::Grammar> {
     ...
     constexpr NonterminalID(Functional::NT _x) : x(uint32_t(_x)) {}
...
namespace Functional {
     ...
     struct Var : public TokenBase<Var> {
          std::string s;
          Var(string s_) : TokenBase<Var>(T::VAR), s(s_) {}
     };
...
inline std::ostream& operator<<(std::ostream& s, Functional::T const& a) {
     return s << Functional::t_strings[uint32_t(a)];
}

inline std::ostream& operator<<(std::ostream& s, Functional::NT const& A) {
     return s << Functional::nt_strings[uint32_t(A)];
}
